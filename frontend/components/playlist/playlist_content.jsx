...
class PlaylistContent extends React.Component {
  ...
  render() {
    ...
    const songList = playlistSongs ? playlistSongs.map((song, id) => {
      return (
        <SongContainer playlistId={this.props.playlist.id} parentIsPlaylist={true} key={id} song={song}/>
    ...
    return (
      <div>
        <section className="pc-section1">
          {this.props.playlist.playlistPhotoUrl
            ? <img className="album-photo" src={this.props.playlist.playlistPhotoUrl}></img>
